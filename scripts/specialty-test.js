...
async function run() {
  ...
  try {
    const fixture = (index, professional, specialization = []) => ({ personalDetails: { firstName: 'Specialty', lastName: `Fixture ${index}`, gender: 'Male', phone: `0170000001${index}`, email: `specialty${index}@example.test` }, professional: { status: 'Active', ...professional }, specialization });
    await Doctor.create([
    ...
    console.log('PASS specialty aliases, all storage fields, active visibility, pagination, counts, legacy route, literal punctuation, and combined search');
